...
class GeofenceQueryService {
  ...
  async getAllLogs(limit = 100, startAfter: string | null = null) {
    try {
  ...
  async getLogsByEventType(eventType: string, limit = 100) {
    try {
  ...
  // Get logs by user ID
  async getLogsByUserId(userId: string, limit = 100) {
    try {
      const snapshot = await db.collection('geofence_essential_logs')
        .where('userId', '==', userId)
        .orderBy('timestamp', 'desc')
  ...
  async getLogsByDateRange(startDate: string, endDate: string, limit = 100) {
    try {
  ...
  async getLogsWithFilters(filters: any = {}, limit = 100) {
    try {
  ...
  return corsHandler(req, res, async () => {
    try {
      ...
      if (path === '/api/logs/date-range' && method === 'GET') {
        ...
        const logs = await queryService.getLogsByDateRange(
          startDate as string, 
          endDate as string, 
